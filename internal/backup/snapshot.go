...
// Snapshotter creates file snapshots before modifications.
type Snapshotter struct {
	now func() time.Time
}

// NewSnapshotter returns a Snapshotter using real wall-clock time.
func NewSnapshotter() Snapshotter {
	return Snapshotter{now: time.Now}
}

// Create snapshots all provided paths into snapshotDir and writes a manifest.
// Files that do not exist are recorded with Existed=false so restore can delete them.
...
func (s Snapshotter) snapshotPath(snapshotDir, sourcePath string) (ManifestEntry, error) {
	...
	if err != nil {
		...
		return ManifestEntry{}, fmt.Errorf("stat %q: %w", cleanSource, err)
	}

	if info.IsDir() {
		// Directories are recorded but not copied — callers must enumerate files.
		return entry, nil
	}

	// Build relative path for the snapshot file.
...
func copyFile(source, destination string, mode os.FileMode) error {
	...
	if err := os.MkdirAll(filepath.Dir(destination), 0o755); err != nil {
		return fmt.Errorf("create backup dir for %q: %w", destination, err)
	}
	...
	if _, err := io.Copy(output, input); err != nil {
		_ = output.Close()
		return fmt.Errorf("copy %q → %q: %w", source, destination, err)
	}
